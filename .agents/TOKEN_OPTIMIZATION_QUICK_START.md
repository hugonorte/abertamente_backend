...
```
.agents/memory/01-architecture.md    ← Directory structure & patterns (MVC/API)
.agents/memory/02-conventions.md     ← Naming & PHP / Eloquent rules
.agents/memory/03-hot-paths.md      ← Which files to modify
.agents/memory/04-tech-decisions.md ← Why each tool, constraints (Laravel ecosystem)
...
```
# Find a Controller or Service
Tool: grep_search (Pattern: "class [NAME]")

# Find API Endpoint Routes
Tool: grep_search (Pattern: "\\[HttpGet", Path: "Controllers/")

# Find Database Entity
Tool: grep_search (Pattern: "class [NAME]", Path: "Models/")

# Find usage of a service
Tool: grep_search (Pattern: "_[name]Service", Path: "Controllers/")

# Find a test
Tool: grep_search (Pattern: "[NAME]", Path: "Tests/", Includes: ["*.cs"])
```
...
|------|-----------|--------------|-----------|
| **"How do controllers work?"** | 02-conventions.md | - | 3K |
| **"Where is the User entity?"** | 03-hot-paths.md | `grep_search: class User` | 1K |
| **"How do I write PHP Eloquent queries?"** | 02-conventions.md | - | 3K |
| **"What's the DI setup?"** | 01-architecture.md | `grep_search: AddScoped in routes/api.php` | 3K |
...
| **"I need to see an example"** | 04-tech-decisions.md | `grep_search: pattern in app/` | 2K |
...
3. Run: laravel CLI commands (0KB)
4. Check: Dockerfile or CI configs (1KB)
...
| Read full service class | 30K | Head/tail or grep (3K) |
...
**Task**: "Add a new endpoint to get active users"
...
├─ Grep for controller example: 1KB
├─ Check existing test pattern: 1KB
└─ Total: 2K tokens

Phase 3: Code (30 min)
├─ Write Controller and Service logic: 0KB
├─ Write tests: 0KB
└─ Total: 0K tokens

Phase 4: Validation (10 min)
├─ Run laravel build / php artisan test: 0KB
├─ Git diff: 1KB
└─ Total: 1K tokens
...
6. **Validate fast** — Run laravel build early to catch syntax errors
...
- [ ] Are tests and build passing? (yes = 0 tokens on debugging)
...
Models/ / Data/                              ← Eloquent / Entities
Tests/                                       ← PHPUnit tests
.env                             ← Config
abertamente-backend.sln                  ← Solution file
```
...
❌ **Build failing** = Didn't follow conventions  
