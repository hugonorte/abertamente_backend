...
This directory now contains a **comprehensive token-saving system** designed to help agents (Claude or any other AI) work efficiently on this Laravel/PHP project.
...
- **`01-architecture.md`** (2KB) — Directory structure, backend organization, MVC / Minimal APIs setup
- **`02-conventions.md`** (3KB) — Naming rules, PHP patterns, DI, Eloquent conventions, testing
...
- **Previous rules** — PHP / Eloquent conventions, security, etc. (unchanged)
...
- ✅ PHP conventions — PascalCase for classes/methods, camelCase for variables
- ✅ Dependency Injection — required for all services and repositories
- ✅ Async/Await — always use async Eloquent methods (`get`, etc.)
- ✅ toBase — mandatory for read-only queries
...
Validation:  1K  (run php artisan test)
────────────────
...
Agent starts task:
  ...
  → Reads entire files for patterns
...
├─ php-ef-conventions.md     (existing)
├─ seguranca.md                 (existing)
└─ ... others
...
**Cache version**: 1.1 (PHP adaptation)  
