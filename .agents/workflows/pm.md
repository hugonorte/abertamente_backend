...
1. **Leitura de Objetivos**: Leia o arquivo `task.md`. Caso o arquivo `task.md` esteja vazio, apenas faça uma revisão na documentação contida nos arquivos na pasta .agents/rules e certifique-se que tudo está de acordo com as especificações gerais do projeto. 
2. **Definição Técnica**: Atue como o **Product Manager** e execute a skill `write_specs.md` para criar ou atualizar os arquivos na pasta .agents/rules.
