...
- **`01-architecture.md`** — Directory structure, component organization, MVC / Minimal APIs setup, Dependency Injection
- **`02-conventions.md`** — Naming patterns, import rules, PHP usage, Eloquent patterns, testing conventions
...
| **Why PHP/Laravel/Eloquent** | `.agents/memory/04-tech-decisions.md` | 5KB |
...
### "I need to write a new API Endpoint / Controller"
1. Read `conventions.md` — Controller structure, routing, DI
2. Grep for a similar Controller to see actual pattern
...
### "I need to understand Eloquent migrations"
1. Read `architecture.md` section on data access
2. Read `tech_decisions.md` section on database rationale
3. Grep for Eloquent Model usage

### "I need to test a service"
1. Read `conventions.md` section on testing
2. Find existing test in `hot_paths.md` (they're usually in `Tests/`)
3. Grep for `[Fact]` or `[Theory]` examples

---

## Constraints (Never Violate)

These are non-negotiable and documented in `tech_decisions.md` / `php-ef-conventions.md`:

- ✅ Eloquent — always use async methods (`get`, `first`)
- ✅ Read-only Queries — mandatory use of `.toBase()`
- ✅ Dependency Injection — Eloquent Model/Repositories/Services always via constructor
- ✅ N+1 Prevention — explicitly use `.Include()` for eager loading related data
- ✅ PHP Naming Conventions — PascalCase for classes/methods/properties, camelCase for params, `_` prefix for private fields
...
- Grep for existing `*Tests.cs` files
...
- Laravel SDK required
- Use `laravel build` and Dockerfiles

---

## Example: Finding All Controllers

**Without cache** (expensive):
```bash
# Agent explores entire Controllers/
# Reads 20+ files
# Consumes 50K tokens
```
...
2. Agent sees list of API endpoints
3. Agent uses `grep_search` for specific controller if needed
4. Total: 1K tokens
